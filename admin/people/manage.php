	...
	<div class="row">
		...
		<div class="col-md-6">
			...
			<div class="form-group">
				...
				<div class="form-group">
				<label for="" class="control-label">Upload Vaccination Certificate</label>
				<div class="custom-file">
		          <input type="file" class="custom-file-input rounded-circle" id="customFile" name="vimg" onchange="displayVImg(this,$(this))" accept=".docx ,.pdf">
		          <label class="custom-file-label" for="customFile">Choose file</label>
		        </div>

			</div>
			<div class="form-group d-flex justify-content-center">
				<img src="<?php echo validate_image(isset($vax_path) ? $vax_path : '') ?>" alt="" id="vcimg" class="img-fluid img-thumbnail">
			</div>
...
<script>
	...
	function displayVImg(input,_this) {
	    if (input.files && input.files[0]) {
	        var reader = new FileReader();
	        reader.onload = function (e) {
	        	$('#vcimg').attr('src', e.target.result);
	        }
